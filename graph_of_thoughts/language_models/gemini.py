...
class Gemini(AbstractLanguageModel):
    ...
    def __init__(
        ...
        self.prompt_chars: float = 0.0
        self.completion_chars: float = 0.0
    ...
    def chat(self, messages: List[Dict]) -> GenerationResponse:
        ...
        try:
            while attempts < 20:  # Limit attempts to 20
                ...
                self.prompt_chars = (
                    float(self.prompt_tokens) * 4
                )  # Assuming 100 tokens are 80 words
                self.completion_chars = (
                    float(self.completion_tokens) * 4
                )  # Assuming 100 tokens are 80 words
                ...
                if response.candidates[0].finish_reason.value in [
                    1,
                    2,
                ]:  # 1 is STOP normal finish_reason and 2 is MAX_TOKENS finish_reason.
    ...
    def get_response_texts(
        ...
        return [response.text for response in query_response]
